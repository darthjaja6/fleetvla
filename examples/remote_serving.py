...
def _robot(listener: socket.socket) -> None:
...
async def _serve(engine: AsyncServingEngine) -> tuple:
    try:
...
def main() -> None:
    with socket.socket() as listener:
        ...
        listener.listen(1)
        robot = threading.Thread(target=_robot, args=(listener,))
        robot.start()
    ...
    print(f"remote session: {observations} observations, {actions} executed actions")
